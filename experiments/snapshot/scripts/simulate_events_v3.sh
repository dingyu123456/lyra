...
# 用法: bash simulate_events_v3.sh <max_rate> <duration> <nodes_per_cluster> <cluster_list>
#
# 核心优化:
#   1. curl 直连 Kubernetes API，绕过 kubectl 进程开销 (~35ms vs ~100ms)
#   2. 批量后台执行，每批 CONCURRENCY 个并发
#   3. 预计算证书路径和 API 端口
# =============================================================================
...
if ! curl -s -o /dev/null -w "%{http_code}" \
    --cacert "${CA_CERTS[0]}" --cert "${ADMIN_CERTS[0]}" --key "${ADMIN_KEYS[0]}" \
    "https://127.0.0.1:${API_PORTS[0]}/api/v1/nodes?limit=1" 2>/dev/null | grep -q "200"; then
    echo "错误: 无法连接到 ${CLUSTER_LIST[0]} API (端口 ${API_PORTS[0]})"
    echo "证书路径: ${CA_CERTS[0]}"
    exit 1
fi
