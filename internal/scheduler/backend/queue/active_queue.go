...
type activeQueuer interface {
	...
	clusterEventsForPod(logger *zap.Logger, pInfo *framework.QueuedPodInfo) ([]*clusterEvent, error)
...
func newActiveQueue(queue *heap.Heap[*framework.QueuedPodInfo], isSchedulingQueueHintEnabled bool, logger *zap.Logger) *activeQueue {
	aq := &activeQueue{
		queue:                        queue,
...
func (aq *activeQueue) pop(logger *zap.Logger) (*framework.QueuedPodInfo, error) {
	aq.lock.Lock()
	defer aq.lock.Unlock()
...
func (aq *activeQueue) unlockedPop(logger *zap.Logger) (*framework.QueuedPodInfo, error) {
	for aq.queue.Len() == 0 {
		// When the queue is empty, invocation of Pop() is blocked until new item is enqueued.
		// When Close() is called, the p.closed is set and the condition is broadcast,
		// which causes this loop to continue and return from the Pop().
		if aq.closed {
			logger.Debug("Scheduling queue is closed")
			return nil, nil
		}
		aq.cond.Wait()
	}
	...
	if aq.isSchedulingQueueHintEnabled {
		...
		if _, ok := aq.inFlightPods[pInfo.Pod.UID]; ok {
			...
			logger.Error("the same pod is tracked in multiple places in the scheduler, and just discard it",
				zap.String("pod", pInfo.Pod.Name),
				zap.String("namespace", pInfo.Pod.Namespace),
			)
...
func (aq *activeQueue) clusterEventsForPod(logger *zap.Logger, pInfo *framework.QueuedPodInfo) ([]*clusterEvent, error) {
	aq.lock.RLock()
	defer aq.lock.RUnlock()
	logger.Debug("Checking events for in-flight pod",
		zap.String("pod", pInfo.Pod.Name),
		zap.String("namespace", pInfo.Pod.Namespace),
		zap.Any("unschedulablePlugins", pInfo.UnschedulablePlugins),
		zap.Int("inFlightEventsSize", aq.inFlightEvents.Len()),
		zap.Int("inFlightPodsSize", len(aq.inFlightPods)),
	)
